...
logger = Logger(logger_folder = r'log',
...
def handel_error(message):
    logger.info("default_error:" + message)
...
class BitgetWsClient:
    ...
    def build(self):
        ...
        while not self.has_connect():
            trying += 1
            logger.info("start connecting... url: {self.__url}")
            time.sleep(1)
    ...
    def connect(self):
        try:
            ...
            print('connect funtion exception :' + ex)
    ...
    def __on_close(self, ws, close_status_code, close_msg):
        logger.info("ws is closeing ......{close_status},{close_msg}")
        self.__close()
        if not self.__reconnect_status:
            self.__re_connect()
...
class BooksInfo:
    ...
    def check_sum(self, new_check_sum):
        ...
        logger.info("start checknum mergeVal:" + str(merge_num) + ",checkVal:" + str(new_check_sum)+",checkSin:"+str(self.__signed_int(merge_num)))
