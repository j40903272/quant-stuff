...
struct Statistics
{
    ...
    Statistics(const std::string &name, StatisticsFunc *compute) : name_(name), compute_(compute)
    {
    }
};
...
class State
{
  public:
    ...
    BENCHMARK_ALWAYS_INLINE
    int64_t complexity_length_n() const
    {
...
#define BENCHMARK_CAPTURE(func, test_case_name, ...)                                               \
    BENCHMARK_PRIVATE_DECLARE(func) = (::benchmark::internal::RegisterBenchmarkInternal(           \
...
class ConsoleReporter : public BenchmarkReporter
{
  public:
    ...
    explicit ConsoleReporter(OutputOptions opts_ = OO_Defaults)
        : output_options_(opts_), name_field_width_(0), prev_counters_(), printed_header_(false)
    {
    }
...
class BENCHMARK_DEPRECATED_MSG("The CSV Reporter will be removed in a future release") CSVReporter
