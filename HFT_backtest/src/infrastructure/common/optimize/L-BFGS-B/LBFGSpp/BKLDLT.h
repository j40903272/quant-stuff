...
// References:
// 1. Bunch, J. R., & Kaufman, L. (1977). Some stable methods for calculating inertia and solving
...
// 4. Ashcraft, C., Grimes, R. G., & Lewis, J. G. (1998). Accurate symmetric indefinite linear
// equation solvers.
...
class BKLDLT
{
  private:
    ...
    std::vector<std::pair<Index, Index>>
        m_permc;  // compressed version of m_perm: [(0, 2), (2, 3), (3, 1)]
    ...
    int gaussian_elimination_2x2(Index k)
    {
        ...
        for (Index j = 0; j < ldim; j++)
        {
            MapVec(col_pointer(j + k + 2), ldim - j).noalias() -=
                (X.col(0).tail(ldim - j) * l1ptr[j] + X.col(1).tail(ldim - j) * l2ptr[j]);
        }
