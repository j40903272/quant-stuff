...
//
// Reference:
// [1] R. H. Byrd, P. Lu, and J. Nocedal (1995). A limited memory algorithm for bound constrained
// optimization. [2] C. Voglis and I. E. Lagaris (2004). BOXCQP: An algorithm for bound constrained
...
class SubspaceMin
{
  private:
    ...
    static bool P_converged(const IndexSet &yP_set, const Vector &vecy, const Vector &vecl,
                            const Vector &vecu)
    {
  ...
  public:
    ...
    static void subspace_minimize(const BFGSMat<Scalar, true> &bfgs, const Vector &x0,
                                  const Vector &xcp, const Vector &g, const Vector &lb,
                                  const Vector &ub, const Vector &Wd, const IndexSet &newact_set,
                                  const IndexSet &fv_set, int maxit, Vector &drt)
    {
        ...
        // std::cout << "New active set = [ "; for(std::size_t i = 0; i < newact_set.size(); i++)
        // std::cout << newact_set[i] << " "; std::cout << "]\n"; std::cout << "Free variable set =
        // [ "; for(std::size_t i = 0; i < fv_set.size(); i++)  std::cout << fv_set[i] << " ";
        // std::cout << "]\n\n";
        ...
        for (k = 0; k < maxit; k++)
        {
            ...
            /* std::cout << "** Iter " << k << " **\n";
            std::cout << "   L = [ "; for(std::size_t i = 0; i < L_set.size(); i++)  std::cout <<
            L_set[i] << " "; std::cout << "]\n"; std::cout << "   U = [ "; for(std::size_t i = 0; i
            < U_set.size(); i++)  std::cout << U_set[i] << " "; std::cout << "]\n"; std::cout << "
            P = [ "; for(std::size_t i = 0; i < P_set.size(); i++)  std::cout << P_set[i] << " ";
            std::cout << "]\n\n"; */
