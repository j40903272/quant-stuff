...
#include "infrastructure/common/eigen/Core"
#include "infrastructure/common/optimize/L-BFGS-B/LBFGSpp/BKLDLT.h"
...
// optimization. [2] R. H. Byrd, P. Lu, and J. Nocedal (1995). A limited memory algorithm for bound
...
class BFGSMat
{
  ...
  public:
    ...
    inline int num_corrections() const
    {
        return m_ncorr;
    }
    ...
    inline bool apply_WtPv(const IndexSet &P_set, const Vector &v, Vector &res,
    ...
    // Returns false if the result is known to be zero
    inline bool apply_PtWMv(const IndexSet &P_set, const Vector &v, Vector &res,
    ...
    inline bool apply_PtWMv(const Matrix &WP, const Vector &v, Vector &res,
                            const Scalar &scale) const
    {
    ...
    // Usually d contains many zeros, so we fist compute number of nonzero elements in A set and F
    // set, denoted as nnz_act and nnz_fv, respectively If nnz_act is smaller, compute W'AA'd = WA'
    // (A'd) directly If nnz_fv is smaller, compute W'AA'd = W'd - WF' * (F'd)
    inline void compute_FtBAb(const Matrix &WF, const IndexSet &fv_set, const IndexSet &newact_set,
                              const Vector &Wd, const Vector &drt, Vector &res) const
    {
    ...
    inline void solve_PtBP(const Matrix &WP, const Vector &v, Vector &res) const
    {
        ...
        for (int j = 0; j < m_ncorr; j++)
        {
            mid.col(m_ncorr + j).segment(m_ncorr + j, m_ncorr - j).noalias() =
                m_theta * (m_permMinv.col(m_m + j).segment(m_m + j, m_ncorr - j) -
    ...
    // P'BQv = -WP * M * WQ' * v
    // Returns false if the result is known to be zero
    inline bool apply_PtBQv(const Matrix &WP, const IndexSet &Q_set, const Vector &v, Vector &res,
                            bool test_zero = false) const
    {
        const int nP = WP.rows();
