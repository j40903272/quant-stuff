...
void nelder_mead(int n, const point_t *start, point_t *solution, fun_t cost_function,
                 const void *args, const optimset_t *optimset)
{
    ...
    point_r.x  = reinterpret_cast<double *>(malloc(n * sizeof(double)));
    point_e.x  = reinterpret_cast<double *>(malloc(n * sizeof(double)));
    point_c.x  = reinterpret_cast<double *>(malloc(n * sizeof(double)));
    centroid.x = reinterpret_cast<double *>(malloc(n * sizeof(double)));
    ...
    simplex_t simplex;
    simplex.n = n;
    simplex.p = reinterpret_cast<point_t *>(malloc((n + 1) * sizeof(point_t)));
    for (int i = 0; i < n + 1; i++)
    {
        simplex.p[i].x = reinterpret_cast<double *>(malloc(n * sizeof(double)));
        for (int j = 0; j < n; j++)
        {
            simplex.p[i].x[j] =
                (i - 1 == j) ? (start->x[j] != 0.0 ? 1.05 * start->x[j] : 0.00025) : start->x[j];
        }
    ...
    while (continue_minimization(&simplex, eval_count, iter_count, optimset))
    {
        ...
        if (shrink)
        {
            for (int i = 1; i < n + 1; i++)
            {
                ...
                {
                    simplex.p[i].x[j] =
                        simplex.p[0].x[j] + SIGMA * (simplex.p[i].x[j] - simplex.p[0].x[j]);
                }
                cost_function(n, simplex.p + i, args);
                eval_count++;
            }
    ...
    solution->x = reinterpret_cast<double *>(malloc(n * sizeof(double)));
...
int continue_minimization(const simplex_t *simplex, int eval_count, int iter_count,
